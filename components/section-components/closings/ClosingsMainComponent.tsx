    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Cierres Mensuales</h1>
            <p className="text-muted-foreground mt-2">Gestiona los cierres de período y revisa el historial</p>
          </div>
          {canEdit && (
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Datos acumulados del período actual (sin cerrar)</CardDescription>
                </div>
              </div>
            ...
            {previewLoading ? (
              ...
              <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">Ingresos Totales (USD)</p>
                  <p className="text-2xl font-bold text-green-500">{formatCurrency(preview.total_revenue_usd)}</p>
                </div>
                <div className="space-y-1">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Historial de Cierres ({closings.length})</CardTitle>
            <CardDescription>Registro de todos los cierres mensuales realizados</CardDescription>
          </CardHeader>
          <CardContent>
            {closingsLoading ? (
              <div className="flex items-center justify-center py-8">
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="hidden sm:table-cell">Ingresos USD</TableHead>
                      <TableHead className="hidden md:table-cell">Miembros</TableHead>
                      <TableHead className="hidden lg:table-cell">Fondos Reset</TableHead>
                  ...
                  <TableBody>
                    {closings.length === 0 ? (
                      ...
                      closings.map((closing) => (
                        <TableRow key={closing.id} className="cursor-pointer" onClick={() => handleViewDetail(closing)}>
                          <TableCell>
                            <div>
                          ...
                          <TableCell className="hidden sm:table-cell font-medium text-green-500">
                            {formatCurrency(closing.total_revenue_usd)}
                          </TableCell>
...
function ComparisonCard({ 
  ...
  return (
    <div className="space-y-1 p-3 rounded-lg bg-muted/50">
      ...
      <p className="text-xl font-bold">{format(current)}</p>
      {change !== null && (
