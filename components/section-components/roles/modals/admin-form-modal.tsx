    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>{admin ? "Modifica la información del administrador" : "Crea un nuevo usuario con acceso administrativo"}</DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="email" type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="correo@gimnasio.com" required />
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <Input id="password" type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} placeholder="••••••••" required={!admin} />
              {admin && <p className="text-xs text-muted-foreground">Deja en blanco para mantener la contraseña actual</p>}
            </div>
          </div>
          <DialogFooter>
